...
| BYOK security            | Tab key, server key, AES-256-GCM opt-in storage, redacted APIs                                                        | Verified    | Leakage regression tests, rotation/deletion guide, telemetry redaction verification                                                 | No key in persistence, checkpoints, logs, traces, or exports                    |
...
| Security                 | CSP, origin guard, scoped repositories, hardened container, Gitleaks                                                  | Partial     | Repository-grounded threat model, dependency/SAST/container gates, rate-limit review, independent security assessment               | No unresolved critical/high issue without documented exception                  |
...
| Documentation            | Required ADRs, evaluation/calibration reports, privacy/accessibility/model cards, draft release notes and demo script | Partial     | Context-validated threat model and independent evidence                                                                             | Every public claim links to current measured evidence                           |
